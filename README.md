...
- **Verification Agent**: A sophisticated agent that:
    1.  **Extracts Claims**: Intelligently identifies distinct, factual claims from the generated text.
    2.  **Web Search**: Uses DuckDuckGo to search for evidence related to each claim.
...
2.  **Verification**: The `VerificationAgent` takes the output from the first agent and begins its process:
    - It first sends the text to an LLM with a specific prompt designed to extract all factual claims and return them as a JSON list.
    - For each extracted claim, it performs a web search using the `duckduckgo-search` library.
...
The script will prompt you to enter a topic. The agents will then take over, and you will see the generation and verification process live in your terminal.
...
$ python main.py
 ...
 Enter a topic for the generative agent to write about: The discovery of penicillin
